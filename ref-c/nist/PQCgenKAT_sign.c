//  [mjos 20230417] added KATNUM (number of test vectors) macro
//	minor modifications to suppress compiler warnings.
...
main()
{
    ...
    unsigned char       msg[33 * KATNUM];   //  changed
    ...
    do {
        if ( FindMarker(fp_req, "count = ") == 0 ||
             fscanf(fp_req, "%d", &count) != 1) {
            done = 1;
            break;
        }
        ...
        if ( FindMarker(fp_req, "mlen = ") == 0 ||
            fscanf(fp_req, "%llu", &mlen) != 1) {
